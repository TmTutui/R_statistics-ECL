...
RMSE_lm = RMSE(housing.test[,p],Y_pred_lm)
RMSE_lm
...
On remarque qu'en ajoutant les termes d'interaction l'erreur a diminué. Cependant le modèle avec interaction est très complexe (157 termes) donc certainement trop variable. L'idée est de chercher un modèle plus prédictif, i.e. avec une RMSE plus faible que 4.836.
...
```{r}
dim(coef.lars(mod_lasso)) # 196 valeurs de lambda différentes et 133 coefficients
# Faire un choix de ligne entre 1 (très forte pénalité) et 196 (faible pénalité)
l = 
sum(coef.lars(mod_lasso)[l,]==0)
#summary(mod_lasso)
par(mfrow =c(1,1))
plot(mod_lasso,cex =1, lwd = 3,col = 1:(p-1))
legend(0,0,names(housing.train[,-p]),col = 1:(p-1),lty = rep(1,p-1),lwd = rep(3,p-1),cex = 0.5)
...
housing.train2 =as.data.frame(scale(cbind(x_extend,housing.train[,p])))
#on garde en mémoire moyenne et écart type des données initiales
mean.train = apply(cbind(x_extend,y),2,mean)
sd.train = apply(cbind(x_extend,y),2,sd)
...
par(mfrow = c(1,1))
plot(lambda,beta_mat[1,],ylim = c(min(beta_mat),max(beta_mat)),pch = 19, cex=0.4,ylab = "coefficients")
...
legend(1000,2.5,row.names(beta_mat),col = 1:p,lty = rep(1,p),cex=0.5,pch =  rep(19,8))
```
...
- Pourquoi n'est-il pas nécessaire ici de mettre en place un modèle avec interaction ? 
...
```{r}
#a completer :
mod_tree_pruned =prune(mod_tree,cp = )
par(mfrow=c(1,1))
...
- Combien d'arbres doit-on considérer ? 
- Quelles sont les variables les plus influentes avec ce modèle ? 
...
summary(mod_boosted,n.trees=3000) # based on the estimated best number of trees
...
On construit maintenant une procédure de bagging. L'idée est de moyenner des arbres CART construits sur des échantillons bootstrap  des données de départ. Un échantillon Bootstrap est un tirage avec remise de N points parmi les N points de l'échantillon initial.
...
for (i in 1:B)
{
  u = sample(1:nrow(housing.train),nrow(housing.train), replace=T)
